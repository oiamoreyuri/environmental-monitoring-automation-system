...
**Motivation:** Currently, all configuration parameters — equipment IDs, alert recipients, responsible person details — are initialized from PropertiesService via `Config.gs`. This is an improvement over hardcoded values, but equipment lists and operational parameters still require a code edit and redeployment to change. Any change requires opening the script editor, modifying `Config.gs`, and running `clasp push`.

**Approach:** Create a `SETTINGS` tab in the Google Sheets spreadsheet. `Config.gs` reads this tab once at initialization and merges the values into the configuration objects dynamically. Changes to equipment lists, alert thresholds, or responsible person details become spreadsheet edits — no code changes required.
...
**Approach:** Add threshold validation inside `onFormSubmit` in `Forms.gs`. If `tempAtual`, `tempMax`, or `umidade` fall outside configurable ranges (defined per equipment zone in the SETTINGS tab), trigger an immediate alert via `Notificacoes.gs` — separate from the completeness alerts already in place. Acceptable ranges are based on ambient temperature control standards per production area.
...
**Approach:** ESP32 microcontrollers with DHT22 or SHT30 sensors, one per equipment station. Each device sends an HTTP POST to a new Apps Script endpoint (`?page=iot`) at configurable intervals. The endpoint validates the payload and appends directly to RAW_DATA via `Forms.gs`.

**Infrastructure already in place:**
- The `FONTE` column in RAW_DATA already accepts `"iot"` as a value — the schema requires no changes
- `WebApp.gs` routing already handles multiple `page` parameters and can be extended with a `doPost` handler for IoT payloads
- `LOG_INTEGRIDADE` and the approval workflow in `Certificado.gs` are agnostic to data source — they operate on RAW_DATA regardless of how records were created

**Remaining work:** ESP32 firmware, `doPost` handler in `WebApp.gs`, device provisioning per equipment station.
...
**Approach:** Create a dedicated Google Workspace account for the Quality department. Transfer ownership of the spreadsheet, script, and Drive folders. Update the WebApp deployments under the new account. Update `RESPONSAVEL_EMAIL` and the approval signature reference in PropertiesService.
