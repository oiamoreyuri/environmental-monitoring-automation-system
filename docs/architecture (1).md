...
## Modular Code Architecture (v2)

The codebase was refactored in v2 from a single monolithic file (`Codigo.js`, ~700 lines) into 11 modules with isolated responsibilities. The original structure mixed HTTP routing, business logic, PDF generation, cryptographic hashing, notifications and trigger management in a single file — creating duplicated functions, accidental scope sharing, and high change-impact radius for any modification.

### Module map

| Module | Responsibility | Key functions |
|---|---|---|
| `Config.gs` | Global constants, PropertiesService, CONFIG initialization | `getConfig()`, `setupPropriedades()` |
| `WebApp.gs` | HTTP routing — no business logic | `doGet()`, `doPost()`, `doGetApi_()` |
| `QrCode.gs` | QR Code scanning flow and redirect pages | `rotearQrCode_()`, `registrarAcessoQr_()` |
| `Forms.gs` | Form submission handling, RAW_DATA normalization | `onFormSubmit()`, `garantirAbaRaw_()` |
| `Pdf.gs` | Monthly PDF generation, Drive organization, business day trigger | `gerarPDFsMensais()`, `verificarEGerarPDFs()` |
| `Utils.gs` | Pure utility functions shared across modules | 14 utility functions, no I/O |
| `Certificado.gs` | Approval certificate lifecycle, PCQI workflow, verification page | `processarAprovacao_()`, `gerarPdfCertificado_()` |
| `Integridade.gs` | SHA-256 hashing, LOG_INTEGRIDADE management | `calcularHash_()`, `registrarIntegridade_()` |
| `Notificacoes.gs` | Completeness alerts and approval notifications | `verificarCompletude_()`, `notificarAprovacao_()` |
| `Triggers.gs` | Trigger setup and teardown — infrastructure only | `configurarTodosTriggers()`, `listarTriggers()` |
| `Dev.gs` | Development utilities, diagnostics — no production logic | `diagnosticoSistema()` |

### Dependency documentation convention

The Apps Script runtime loads all `.gs` files into a shared global scope — there is no native import system. Inter-module dependencies cannot be enforced by the runtime. Each module documents its external dependencies explicitly in the file header:

```javascript
// DEPENDÊNCIAS EXTERNAS:
//   Utils.gs → chaveMesAno_, mesFormatado_
//   Pdf.gs   → localizarOuCriarSubpasta_
```

This makes dependency relationships traceable without requiring a build system. Migration to explicit namespace objects (e.g. `PdfService.localizarOuCriarSubpasta()`) is planned for when the project enters a maintenance phase or a second developer joins.

### Bugs corrected during refactoring

Three latent bugs were identified and corrected during the v2 refactoring:

**`CONFIG` never initialized as a global variable.** `getConfig()` existed but its return value was never assigned to `CONFIG`. All `CONFIG.*` references throughout the codebase depended on an undefined global variable. The system appeared to work because Apps Script's `PropertiesService` was being called on every function invocation via the unassigned function reference — a behavior that masked the bug until execution context changed. Fixed in `Config.gs` with `var CONFIG = getConfig()`.

**`gerarPdfCertificado_` nested inside `processarAprovacao_`.** The function was defined inside the `try/catch` block of `processarAprovacao_`, making it a local function invisible at global scope. `gerarPDFsMensais` called it as a global, which worked only due to accidental V8 hoisting behavior of function declarations inside blocks. This is undefined behavior in strict mode and a latent bug in any runtime that changes hoisting semantics. Fixed in `Certificado.gs` as an explicit global function.

**`notificarAprovacao_` defined twice.** One definition was nested inside `gerarPDFsMensais` (unreachable from outside), another at global scope. The nested version was dead code. Both definitions had diverged slightly in content, creating a maintenance hazard. The nested version was removed; only the global version in `Notificacoes.gs` remains.

### Optimizations applied

**Single blob read for hash and size.** The original called `blob.getBytes()` twice per PDF — once inside `calcularHashPDF_` and once to record `TAMANHO_BYTES`. In `Integridade.gs`, bytes are read once in `registrarIntegridade_()` and passed to `calcularHash_()`, which now operates on raw bytes rather than a Blob object. This also decouples the hash function from the Apps Script Blob type, making it reusable with any byte array.

**RAW_DATA as the single source of truth for completeness checks.** `verificarCompletude_()` previously read from "Respostas ao formulário 1" — the raw Forms response sheet — while the rest of the system operated on RAW_DATA. This inconsistency meant completeness alerts could miss records that had been normalized and written to RAW_DATA but differed in format from the raw sheet. Fixed in `Notificacoes.gs` to read exclusively from RAW_DATA.

**Utility functions centralized in `Utils.gs`.** Fourteen functions that were duplicated or scattered across modules — date formatting, decimal normalization, name normalization, filename construction — were extracted into `Utils.gs` as pure functions with no I/O dependencies. Filename construction functions (`nomeArquivoRelatorio_`, `nomeArquivoCertificado_`) eliminate the inline string pattern that was assembled in three different places with no guarantee of consistency.
...
- **Externalized configuration pending**: `CONFIG` and `ALERTA` objects are initialized from PropertiesService but equipment lists and operational parameters still require a code edit and redeployment to change. *(Planned: extract all configuration parameters to a dedicated SETTINGS tab in Google Sheets, making the system fully configurable without touching the codebase)*
